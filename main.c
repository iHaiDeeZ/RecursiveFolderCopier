...
#include <limits.h>
int copying_folders(char *foldercopied, char *path,char desiredpath2[]);
void openandclosedir(char filepath[], char desiredpath[]);
//void openandclosedir2(char *argn,char* filepath);
void copying_file(char *filecopied,char *argm,char desiredpath1[]);
...
int main(int argc, char* argv[] ){
  ...
  char originalpath[512],desiredpath[512];

	//Messages to be printed
    printf("===============================================\n");

  printf("| Welcome to RecursiveFolderCoper by @ShiroKlein |\n");

   printf("===============================================\n");
   printf("1. Enter the desired to be copied folder: \n");
   gets(originalpath);
   printf("2. Enter the destination folder: \n");
   gets(desiredpath);
  	printf("\n\n processing...\n");
  	
		openandclosedir(originalpath,desiredpath);
			...
			fprintf(Tut1,"%s\n Znullptr is gay ",Arr);*/
        		...
        		while((shit=readdir(folder))!=NULL){
    				...
    				if((isFile(currentPath,shit->d_name) == 1) ){
     					...
     					copying_file(shit->d_name,currentPath,desiredpath);
    				}
    				else{
            			printf("%s\t",shit->d_name);
                  //printf("%s\t",shit->d_type);
     					printf("This is a direcotry\n") ;    
                      copying_folders(shit->d_name,currentPath,desiredpath);    		
            						}
...
void copying_file(char *filecopied,char *argm,char desiredpath1[])
{
  ...
  char kpath[260]="",rpath[260]="";
    ...
    strcat(kpath,desiredpath1);
    strcat(kpath,"/");
    strcat(kpath,filecopied);
}
else{ 

  strcat(rpath,filecopied);

  sprintf(kpath,"%s/%s",desiredpath1,filecopied);
...
int copying_folders(char *foldercopied, char *path,char desiredpath2[])
{
char *folderpath=desiredpath2;
  char tpath[512],t2path[512];
  if (path !="."){
